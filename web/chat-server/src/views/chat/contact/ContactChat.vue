  ...
  setup() {
      ...
      store.state.socket.onmessage = (jsonMessage) => {
        ...
        if (message.type != 3) {
          if (
            ...
            // 发送 ACK 回执确认，告知服务端消息已被收到
            sendAck(message.uuid);
            scrollToBottom();
          }
    ...
    const handleShowUserSessionList = async () => {
      try {
        ...
        const userSessionListRsp = await http.post(
    ...
    const handleShowGroupSessionList = async () => {
      try {
        data.ownListReq.owner_id = data.userInfo.uuid;
        const groupSessionListRsp = await http.post(
    ...
    // ACK 回执发送：告知服务端消息已被前端成功接收
    const sendAck = (msgUuid) => {
      if (store.state.socket && store.state.socket.readyState === WebSocket.OPEN && msgUuid) {
        const ackFrame = {
          type: 4,  // Ack 类型
          content: msgUuid,  // 待确认的消息 UUID
          session_id: "",
          url: "",
          send_id: data.userInfo.uuid,
          send_name: "",
        ...
        store.state.socket.send(JSON.stringify(ackFrame));
      }
    };
