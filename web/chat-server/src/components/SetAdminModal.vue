<template>
  <div style="height: 100%; width: 100%">
    ...
    <div class="footer">
      <el-button type="primary" @click="handleSetAdmin">设为管理员</el-button>
      <el-button type="primary" @click="handleCancelAdmin">取消管理员</el-button>
    </div>
  </div>
</template>
  ...
  setup() {
    ...
    const handleSetAdmin = async () => {
      try {
        ...
        await http.post("/user/setAdmin", req);
        getUserInfoList();
      } catch (error) {
        console.log(error);
      }
    ...
    const handleCancelAdmin = async () => {
      try {
        const req = { uuids: data.uuidList };
        await http.post("/user/cancelAdmin", req);
        getUserInfoList();
      } catch (error) {
        console.log(error);
      }
