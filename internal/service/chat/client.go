...
func (c *Client) Read() {
	...
	for {
		...
		if err != nil {
			...
			// ACK 回执处理：前端收到消息后回传 type=4 (Ack)，携带 uuid 字段
			// 服务端收到后将该消息状态从 Sent 更新为 Acked，不再转发到 Kafka
			if message.Type == message_type_enum.Ack {
				ackUuid := message.Content // 前端将待确认的消息 UUID 放在 content 字段
				if ackUuid != "" {
					if res := dao.GormDB.Model(&model.Message{}).Where("uuid = ?", ackUuid).Update("status", message_status_enum.Acked); res.Error != nil {
						zlog.Error(res.Error.Error())
					} else {
						zlog.Info("ACK confirmed for message: " + ackUuid)
					}
				}
				continue // ACK 消息不需要进入 Kafka 管道
			}
